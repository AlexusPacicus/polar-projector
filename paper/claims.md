...
| C15 | Costs of §3.5, one operation per column for every arm (bench/reanchor.py, "Cost columns"). Per query, re-centring costs 16.3 µs for the re-anchored operator (codebook scan + prepare) against 3,889 µs for the local PCA (neighbour scan + SVD), 238× on unrounded timings; the single-view arms and radial do no per-query work. Built once over the corpus, the re-anchoring codebook costs 581 ms, 9.1× the global PCA's 63.7 ms. Per stimulus: 4.64 µs operator, 1.52–1.56 µs PCA, 1.97 µs radial. A full 2,221-point view refresh costs 3.67 ms re-anchored against 5.81 ms local PCA and 1.99 ms radial when vectorized, and 10.4 / 7.2 / 4.3 ms placing points one at a time. Supersedes the old frame_us column (42.8 / 5,156 / 69,178 µs, 121×, 1,618×), which mixed frame-only and frame-plus-view timings and left the neighbour scan out of the local PCA | checked | no (d12e03f); cost columns redefined 2026-09-16 after the audit, recalls unchanged bit for bit | reanchor.json | `§3.5 cost` |
